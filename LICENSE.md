# Rift Source-Available Project License v2.0 (RSAPLv2.0)

**This license applies to “Saphir”.**

**GitHub repository : https://github.com/rifts-minecraft-laboratory/fabric-saphir.**

This software and its source code are distributed under the terms of this license.

## Terms of Use

1. **You may use, copy, modify, and redistribute this project (source-code form and compiled form)**, provided that you comply with the terms of this license.
2. **This license applies whenever this project is used, copied, modified, or redistributed**, whether in source-code form or compiled form.
3. **You must include this license with all distributed versions of the project**.
4. **You must include a visible credit along with a reference to the GitHub repository** (e.g., in the project description, its documentation, or its credits section).
5. **You may charge for a larger project that uses this project’s source code**, provided that the charge is primarily for your own original work (e.g., features or services), and not for this project’s source-code form itself.
6. **You may charge for products or services that use the compiled software**, provided that the charge is primarily for your own original work (e.g., features or services), and not for this project’s compiled form itself.
7. **This license does not prevent you from combining this project with other projects**, as long as the terms of this license are respected for the parts derived from this project.
8. When this project is distributed together with other software or materials, **you must comply with the terms of this license and with the terms of any other applicable licenses**.
9. **If the project to which this license is attached is built on another project** (for example, a Minecraft mod is built on the Minecraft game)**, the permissions granted by this license are limited by the terms of that other project's license or EULA.** Where those terms are more restrictive, they take precedence over this license. This license does not grant any right to violate such other terms.

## Liability

THE SOFTWARE IS PROVIDED “AS IS”, WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS OR COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THE SOFTWARE.

___

(c) 2026 Noah Boos (Propzonzon) (Rift)

Ce projet est protégé par le droit d’auteur. This project is protected under French Copyright Law.